...
Adafruit_PCD8544_Tiny::Adafruit_PCD8544_Tiny(int8_t SCLK, int8_t DIN, int8_t DC,
    int8_t CS, int8_t RST) : Adafruit_GFX_Tiny(LCDWIDTH, LCDHEIGHT) {
  _din = DIN;
  _sclk = SCLK;
  _dc = DC;
  _rst = RST;
...
Adafruit_PCD8544_Tiny::Adafruit_PCD8544_Tiny(int8_t SCLK, int8_t DIN, int8_t DC,
    int8_t RST) : Adafruit_GFX_Tiny(LCDWIDTH, LCDHEIGHT) {
  _din = DIN;
  _sclk = SCLK;
  _dc = DC;
  _rst = RST;
...
void Adafruit_PCD8544_Tiny::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
uint8_t Adafruit_PCD8544_Tiny::getPixel(int8_t x, int8_t y) {
  if ((x < 0) || (x >= LCDWIDTH) || (y < 0) || (y >= LCDHEIGHT))
    return 0;
...
void Adafruit_PCD8544_Tiny::begin(uint8_t contrast) {
  // set pin directions
  pinMode(_din, OUTPUT);
...
inline void Adafruit_PCD8544_Tiny::fastSPIwrite(uint8_t d) {
...
inline void Adafruit_PCD8544_Tiny::slowSPIwrite(uint8_t c) {
  shiftOut(_din, _sclk, MSBFIRST, c);
}

void Adafruit_PCD8544_Tiny::command(uint8_t c) {
...
void Adafruit_PCD8544_Tiny::data(uint8_t c) {
...
void Adafruit_PCD8544_Tiny::setContrast(uint8_t val) {
  if (val > 0x7f) {
    val = 0x7f;
  }
  command(PCD8544_FUNCTIONSET | PCD8544_EXTENDEDINSTRUCTION );
...
void Adafruit_PCD8544_Tiny::display(void) {
...
// clear everything
void Adafruit_PCD8544_Tiny::clearDisplay(void) {
